...
response = requests.get(url = f'http://{server_ESOR}/info.v15.csp?OpCode=5&CSPID=TEST&BSPID=ESOR&RqUID=&CustID=24359')
#  http://10.1.4.22/sirena-ordynsk/info.v15.csp?OpCode=5&CSPID=TEST&BSPID=ESOR&RqUID=&CustID=26959
...
def test_check_CustID():
    for element in root.iter(tag='CustID'):
        assert element.text == '24359', 'Договор не 24359'

def test_check_AcctID():
    for element in root.iter(tag='AcctID'):
        assert element.text == '5005016', 'Лицевой счет не 5005016'
...
def test_check_number_flat():
    for element in root.iter(tag='Flat'):
        assert element.text == '13', 'Квартира не 13'
