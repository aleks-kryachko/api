...
response = requests.get(url = f'http://{server}/sirena/info.v15.csp?OpCode=44&TimeStamp=2020-06-06&RPNum=1&ProductCode=1&ServiceCode=1&CustID=599&AcctID=30806030&QValue=0500')
#  http://10.0.0.202/sirena/info.v15.csp?OpCode=44&TimeStamp=2020-06-06&RPNum=1&ProductCode=1&ServiceCode=1&CustID=599&AcctID=30806030&QValue=0500
...
def test_check_Status_3():
    for element in root.iter(tag='Status'):
        assert element.text == '0 (-3):Показания по прибору учета №094392061 на текущую дату уже есть в системе.', 'Данные не передались'

def test_check_Status_2():
    date_today = datetime.datetime.today().strftime('%Y-%m-%d')

    response = requests.get(
        url=f'http://{server}/sirena/info.v15.csp?OpCode=44&TimeStamp={date_today}&RPNum=1&ProductCode=1&ServiceCode=1&CustID=599&AcctID=30806030&QValue=9999999')
