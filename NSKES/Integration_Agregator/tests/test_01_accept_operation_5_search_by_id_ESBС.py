...
response = requests.get(url = f'http://{server_ESBC}/info.v15.csp?OpCode=5&CSPID=TEST&BSPID=ESBC&RqUID=&CustID=26959')
#  http://10.1.4.22/sirena-chulym/info.v15.csp?OpCode=5&CSPID=TEST&BSPID=ESBC&RqUID=&CustID=26959
...
def test_check_CustID():
    for element in root.iter(tag='CustID'):
        assert element.text == '26959', 'Договор не 26959'

def test_check_AcctID():
    for element in root.iter(tag='AcctID'):
        assert element.text == '200245', 'Лицевой счет не 200245'
...
def test_check_number_house():
    for element in root.iter(tag='House'):
        assert element.text == '33', 'Дом не 33'
