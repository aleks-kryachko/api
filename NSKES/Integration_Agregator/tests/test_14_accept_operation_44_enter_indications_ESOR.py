...
response = requests.get(url = f'http://{server_ESOR}/info.v15.csp?OpCode=44&TimeStamp=2020-06-06&RPNum=1&ProductCode=1&ServiceCode=1&CustID=101000000000012349&AcctID=5005016&QValue=9999999')
#  http://10.1.4.22/sirena-ordynsk/info.v15.csp?OpCode=44&TimeStamp=2020-06-06&RPNum=1&ProductCode=1&ServiceCode=1&CustID=101000000000012349&AcctID=5005016&QValue=9999999
...
def test_check_Status_5():
    for element in root.iter(tag='Status'):
        assert element.text == '0 (-3):Показания по прибору учета №114101323 на текущую дату уже есть в системе.', 'Данные не передались'

def test_check_Status_2():
    date_today = datetime.datetime.today().strftime('%Y-%m-%d')

    response = requests.get(
        url=f'http://{server_ESOR}/info.v15.csp?OpCode=44&TimeStamp={date_today}&RPNum=1&ProductCode=1&ServiceCode=1&CustID=101000000000012349&AcctID=5005016&QValue=9999999')
