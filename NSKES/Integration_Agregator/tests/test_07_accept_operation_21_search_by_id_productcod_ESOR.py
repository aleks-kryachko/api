...
response = requests.get(
        url=f'http://{server_ESOR}/info.v15.csp?OpCode=21&CSPID=TEST&BSPID=ESOR%D0%A1&RqUID=&AcctID=0101100&ProductCode=6')
        #  http://10.1.4.22/sirena-ordynsk/info.v15.csp?OpCode=21&CSPID=TEST&BSPID=ESOR%D0%A1&RqUID=&AcctID=0101139&ProductCode=6
...
def test_check_OrgCode_ProductCod_6():     #  OrgCode и продукция 6
    for element in root.iter(tag='OrgCode'):
        assert element.text == '141', 'OrgCode  не 141'

def test_check_OrgCode_ProductCod_14():     # ЛС 2408104 OrgCode и продукция 14
    response = requests.get(
        url=f'http://{server_ESOR}/info.v15.csp?OpCode=21&CSPID=TEST&BSPID=ESOR%D0%A1&RqUID=&AcctID=2408104&ProductCode=14')
    root = ET.fromstring(response.content)
    for element in root.iter(tag='Count'):
        assert element.text == '1', 'Нет информации для вывода. Ответ Пуст'
    for element in root.iter(tag='OrgCode'):
